...
namespace WebApi.Controllers
{
    ...
    public class WebController : ApiController
    {
        ...
        [Route("login2")]
        [HttpGet]
        public DtoTaskForClient Login2(string name, string pwd)
        {
            var modelClient = new DtoTaskForClient();
            var model = new DtoTask();
            try
            {
                ...
                if (clientUserId > 0)
                {
                    ...
                    model.Ip = clientIp;
                    model.Memo = "";
                    Lfb.DataGrabBll.DalNews.Insert_Task2(model);
                }
                else
                {
                    throw new Exception("账号密码错误");
        ...
        [Route("update2")]
        [HttpGet]
        public bool Update2(string taskId, int result)
        {
            try
            {
                if (result >= 1)
                {
                    ...
                    result = 0;
                return Lfb.DataGrabBll.DalNews.UpdateTaskStatus2(taskId, result);
